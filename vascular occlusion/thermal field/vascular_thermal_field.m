...
delta_r=dx;   %1e-4;
delta_x=dz;  %1e-4;
% xx=x(203:459);   %只取血管附近部分 x =±0.032
xx=x(83:579);   %只取组织部分 x =±0.062
% xx=x(42:290);   %只取组织部分 x =±0.062
% zz=z(121:281);  %muscle:100points blood20:points muscle:40points
...
N_repet=2; % number of repetition including heat and cool duration 
t_cool = 5;	% cool-off duration (s)关闭辐照,冷却时间
% % % periodic sonications
n_cycle = 20;		% number of pulse cycles 脉冲循环数
D = 50;		% duty cycle (%) 占空比
t_pulse = 1;	% pulse cycle period (s) 脉冲循环时长
% 
...
if(n_cycle==0)
  ...
  N_p = round(t_pulse/delta_t);
end
N_c = round(t_cool/delta_t);
N = n_cycle*N_p+N_c;		% total number of integration steps
...
for n=n_cycle*N_p+1:N
    input(n) = cooloff(n-n_cycle*N_p);
end
input_all=[];
for m=1:N_repet
...
V0=0;   % average blood flow velocity 30cm/s
theta=pi/2;
...
% Kt_multi=medium6.thermalconductivity;
...
% Wb_multi=medium6.bloodperfusion;
Wb_blood=medium5.bloodperfusion;
Wb_muscle=medium6.bloodperfusion;

%index of interface
index_tissueblood=size(Ct_multi,1);%=100;
...
f1 = c1;
  ...
  c2(r)= -1 / (12 * (r + 0.2) * delta_r^2);
  
  d2(r)= 2 / (3 * (r + 0.2) * delta_r^2);

  e2(r)= -2 / (3 * (r + 0.2) * delta_r^2);
    
  f2(r)= 1 / (12 * (r + 0.2) * delta_r^2);
% generate matrix A
...
Damage=zeros(gridNum_x,gridNum_r);
% calculate T 
for i=1:times
...
%calculate mulilayer thermal field
    ...
    T2_multi =( m_multi .* (T1_multi * A + q_multi .* T1_multi) + n_multi .* (-Tp_2_multi + 16 * Tp_1_multi + 16 * Tm_1_multi - Tm_2_multi)...
            +o_multi .* Ta_multi +4*T1_multi -T0_multi + input_all(i)*s_multi .* abs(pressure_xr_multi).^2)./3; %去掉血流灌注不考虑+o * Ta
...
% 
%     Tp_2_blood = [T1_blood(3:nx_blood,:);T1(index_bloodmuscle+1:index_bloodmuscle+2,:)];
%     Tp_1_blood = [T1_blood(2:nx_blood,:);T1(index_bloodmuscle+1,:)];
%     Tm_1_blood = [T1(index_tissueblood-1,:);T1_blood(1:nx_blood - 1,:)];
%     Tm_2_blood = [T1(index_tissueblood-2:index_tissueblood-1,:);T1_blood(1:nx_blood - 2,:)];    
    ...
    T2_blood =( m_blood * (T1_blood * A + q_blood * T1_blood) + n_blood * (-Tp_2_blood + 16 * Tp_1_blood + 16 * Tm_1_blood - Tm_2_blood)...
            ...
            +4*T1_blood -T0_blood + input_all(i)* s_blood * abs(pressure_xr_blood).^2)./3; 
%              +4*T1_blood -T0_blood + s_blood * abs(pressure_xr_blood).^2)./3; 
% % def clearBoundary(T2_blood):
%     T2_blood(1,:) = T2_multi(nx_multi,:);
...
%calculate muscle thermal field
    ...
    T2_muscle =( m_muscle * (T1_muscle * A + q_muscle * T1_muscle) + n_muscle * (-Tp_2_muscle + 16 * Tp_1_muscle + 16 * Tm_1_muscle - Tm_2_muscle)...
            +o_muscle * Ta_muscle  + 4*T1_muscle -T0_muscle + input_all(i)*s_muscle * abs(pressure_xr_muscle).^2)./3;
%     end
...
Damage=Damage+exp(-Ea./(R_constant*T2))*delta_t;
...
% hold on;
% scatter(z_index,round(gridNum_r/2),1,50,'w','*');
figure();
plot(T_focus);
figure();
contourf(ThermalDamage',[5,5]);
figure ()
surf(ThermalDamage);
shading interp;
axis equal;
