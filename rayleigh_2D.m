function [output] = rayleigh_2D(R,a,u,x,z)
%RAYLEIGH_2D 对换能器的面积进行划分得到dS,使用rayleigh积分计算声压。输入R,a,u,x,z
...
for ix=1:nx  %观察网格点x方向的坐标
    for iz=1:nz  %观察网格点z方向的坐标
        ...
        A=dS.*exp(-1i.*k.*rn)./rn.*u;
        B=sum(sum(A));%对上述求得的值累加
        pr(ix,iz)=1i*medium.density*medium.soundspeed*k/(2*pi)*B; %乘以相关参数得到声压p
    end
end
toc
