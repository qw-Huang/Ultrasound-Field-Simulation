%使用自定义rayleigh积分求三维空间中的声强分布,对比自定义rayleigh积分和focus内置rayleigh积分+ASA的感兴趣体误差  
clc;
clear all;
clear all;
...
I_pre_asa=acousticintensity(pre_asa,medium.density,medium.soundspeed); %声强计算
toc
...
I_pre_max=max(I_pre_asa(:));%自定义rayleigh 3D声强最大值
I_pr_max=max(max(I_pr(:)));

I_pr_nor=I_pr./I_pr_max;
I_pre_nor=I_pre_asa./I_pre_max;
error_I=abs(I_pre_nor-I_pr_nor)./I_pre_nor;%计算三维空间中两种方法的误差
...
xlabel('y');
ylabel('x');
title('rayleigh 3D VS Rayleigh(FOCUS)+ASA 焦点处xy平面误差情况');
...
xlabel('z');
ylabel('x');
title('rayleigh 3D VS Rayleigh(FOCUS)+ASA 焦点处xz平面误差情况');
...
% xlabel('z');
% ylabel('error');
% title('rayleigh 3D VS FNM+ASA 感兴趣体xy截面最大误差的变化情况');
% figure(7);
% plot(zz,error_average);
% xlabel('z');
% ylabel('error');
% title('rayleigh 3D VS FNM+ASA 感兴趣体xy截面平均误差的变化情况');
