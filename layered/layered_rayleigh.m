...
% k1=2*pi*f0/medium(1).soundspeed-j*medium(1).attenuationdBcmMHz;%波数
% k2=2*pi*f0/medium(2).soundspeed-j*medium(2).attenuationdBcmMHz;%波数
...
% for ix=1:nx  %观察网格点x方向的坐标
    for iz=1:nz  %观察网格点z方向的坐标      
        rm2=sqrt((X1-0).^2+(Y1-0).^2+(Z1-z(iz)).^2);%interface1到点源的距离
