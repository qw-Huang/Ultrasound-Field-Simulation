...
for i=1:length(R)
    a(i)=R(i)/(2*fnumber);
    d(i) = sqrt(R(i)^2 - a(i)^2);
u=normal_velocity(P,R(i),a(i),hole_a(c),medium1.density,medium1.soundspeed);
z0=R(i)-d(i)+lambda;
z_interface1=R(i)-depth;
...
xdcr_array_hole = get_spherical_shell(hole_a(c),R(i));
...
[x_index,y_index,z_index]=ind2sub(s,focus_index_1);%将最大值单下标转为三维多下标
    ...
    focus_forward(c,i)=R(i)-z5(focus_index_z(i));
%轴向-6dB
...
% p_asa4(:,:,1)=[];
% p_asa5(:,:,1)=[];
% p_asa=[p_asa1 p_asa2 p_asa3 p_asa4 p_asa5];
% z2(1)=[];z3(1)=[];z4(1)=[];z5(1)=[];
% z=[z1 z2 z3 z4 z5];
end
end
